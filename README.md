...
Both the local and docker methods use the _ZFS-root-packerci.conf_ config file for **ZFS-root.sh** to provide all the information for a local install.  The username/password is `packer/packer`.
...
A sample run from the repo directory with docker would be (replace *myname* with your user or other identifier)
